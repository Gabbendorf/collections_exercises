...
RSpec.describe Hashes do
  ...
  it "returns an hash with all numbers as keys and their occurence as value" do
    expect(hashes.occurences).to eq({12 => 3, 42 => 1, 75 => 1, 24 => 1, 8 => 1, 7 => 1, 1 => 1, 9 => 1, 36 => 1, 14 => 1, 85 => 1})
  end

  it "returns numbers with occurence = 3" do
    expect(hashes.get_common_favorite_numbers).to eq([12])
  end
